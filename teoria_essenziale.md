...
L’insieme delle parti ${ \mathcal { P } } ( A )$ di $A \ è$ è l’insieme di tutti i possibili sottoinsiemi di $A$. 
...
$$
\cup_ {i = 1} ^ {k} B _ {i} = \Omega \quad B _ {i} \cap B _ {j} = \emptyset \space ,  \forall \space i \neq j
$$
...
Essendo $B _ { i } \cap B _ { j } = \emptyset$, avremo ovviamente che $( A \cap B _ { i } ) \cap ( A \cap B_ { j } ) = \emptyset$, per cui:
...
c. Se $A$ è un evento qualsiasi, allora la minima algebra che contiene $A$ è $\mathcal { E } = \{ A , \overline { { A } } , \Omega , \emptyset  \}$. Infatti: 
...
>
> 	3a. **Numerabile additività**. Se $\{ B _ { n } \} _ { n \in \space \mathbb { N } } \ è$ una collezione numerabile di eventi incompatibili, allora: 
>
>$$
...
Nel caso [b] vale la precedente relazione per tutti i punti di $Y$ in cui $g ( y ) \ { è }$ invertibile. Per un punto $y _ { k }$ tale che $g ( x _ { 1 } ^ { ( k ) } ) = g ( x _ { 2 } ^ { ( k ) } ) = \ldots = g ( x _ { { L _ { k } } } ^ { ( k ) } ) = y _ { k }$ avremo: 
