...
- Vista uniforme sull’informazione memorizzata in memoria non volatile.
...
- Contenuti definiti dal creatore del file:
    - Diversi tipi (Considera text file, source file, executable file).
...
Su disco viene mantenuto un indice del file (**inode** in Unix/Linux) con i metadata del file.
- **Non il nome del file**, delegato alle directory entries.

## Operazioni File

Il File è un tipo di dato astratto. Occorre definire il tipo di operazioni (associate a **system call**):
...
**Open-File Table**: La SO traccia tutti i file aperti; i processi hanno info sui loro file aperti (due tabelle, una per-processo e una di sistema).
...
Fornito da alcuni SO e file system (Simile a read-write locks):

- **Shared lock**: simile a reader lock – molti processi possono acquisire concorrentemente.
- **Exclusive lock**: simile a writer lock.

In generale sul `write(fd, data, n)` ci può essere race condition, va gestita dal processo.
...
## Metodi di Accesso (Modello Disco)
...
## Tree-Structured Directory (Proprietà)
...
## Tree-Structured Directories (Operazioni)

- path name *assoluti* o *relativi*
- Creazione di un nuovo file fatta nella directory corrente.
- Cancellazione di file: `rm <file-name>`
- Creazione di una nuova subdirectory è fatto nella directory corrente: `mkdir <dir-name>`
...
- Se si mantengono accesso a file non esistenti possibili problemi:
    ...
    - Si possono cercare tutti i link associati ad un file ed eliminarli (Occorre memorizzare per ogni file i riferimenti al file, costoso).
- **Soluzione con contatore**:
    - Si contano i link per file, quando il contatore va a zero si può concellare il file.

## General Graph Directory

## General Graph Directory (Cicli)
...
Esempio: Il file 107 ha 3 links.
...
*Diagram showing the UNIX File System with components such as Dischi, Stampanti, Nastri, Terminali, Linee di comunicazione, and Pipes. The UNIX File System is connected to the Comandi UNIX e Programmi utente via a stessa interfaccia per ogni tipo di file.*

## Implementazione File

- Directory
- PIPPC 102
- 102 attributi del file
...
## Accessi a Liste e Gruppi (Riepilogo)
...
- Attacca un gruppo ad un file: `chgrp G game`
...
- Semplice: richiesta locazione di partenza (numero del blocco) e lunghezza (numero di blocchi).
- **Pro**:
...
- **Contro**:
    ...
    - Necessità di compattare:
        - off-line è downtime.
        - on-line è lento.
- **Allocazione contigua estesa**: Aree contigue collegate (base, numero blocchi, base ext, numero blocchi ext).
...
## Metodi di Allocazione - Contigua (Pro e Contro)
...
- **Contro 1**: è necessario trovare lo spazio libero per allocare il file.
    - Diversi algoritmi: **First-fit** (primo buco abbastanza grande), **Best-fit** (il buco che approssima meglio la dimensione per eccesso).
...
## Metodi di Allocazione - Contigua (Limiti e Estensioni)

- **Pro**: l’accesso ai file può essere eseguito semplicemente sia in modo sequenziale che diretto (meno veloce).
- **Contro 2**: è necessario stimare quanto spazio allocare ad un file.
    - Quando un file finisce lo spazio disponibile: il processo può essere interrotto oppure il file può essere spostato in una zona di memoria più grande.
    - Allocare più spazio di quello inizialmente necessario può portare alla **frammentazione interna** (specialmente in file che crescono lentamente).
- Alcuni sistemi usano l’**allocazione contigua estesa**:
    ...
    - Ogni area è caratterizzata da: blocco d’inizio, numero blocchi, area successiva (base, numero blocchi, base ext, numero blocchi ext).
...
- Un **extent** è un blocco contiguo di memoria.
    ...
    - Un file consiste di uno o più estensioni: prima viene allocato un chunk, poi vengono aggiunti altri (base, numero blocchi, base ext, numero blocchi ext).
...
## Allocazione Concatenata (Operazioni)
...
## Allocazione Concatenata (Analisi)
...
- I puntatori consumano spazio.
    - Una soluzione è quella di allocare **cluster di blocchi** anziché singoli blocchi (meno spazio sprecato per i puntatori, meno spostamenti della testina; aumenta però la frammentazione interna).
...
## File-Allocation Table (Dettagli)
...
## File-Allocation Table (Efficienza)
...
## File-Allocation Table (Dimensioni)
...
## Allocazione Indicizzata (Operazioni)
...
## Allocazione Indicizzata (Soluzioni)
...
## Schema Combinato: UNIX UFS (32-bit)
...
## Scelta Metodo Allocazione (Ottimizzazioni)
...
## Gestione dello Spazio Libero (Implementazioni)
...
## Linux File System (Evoluzione Dettagliata)
...
- Tuttavia questo richiede due accessi al disco ogni volta che si legge un file (uno in lettura e uno in scrittura per modificare la directory).
- Gli attributi da memorizzare vanno scelti con cura.

**Lunghezza dei puntatori per accedere ai dati**:
- Più bit si usano per i puntatori più grandi possono essere i file (32 bit = $2^{32} = 4$GB).
...
- In caso di accesso sequenziale LRU non è ottimale (la pagina più recente non è quella che sarà usata prima).
...
- **Consistency checker** confronta i dati nelle directory e altri metadata con i dati memorizzati e prova a correggere:
    - Con l’allocazione concatenata i puntatori permettono di ricostruire un file (Si possono ricreare i file in una directory).
    - Con allocazione indicizzazione grave la perdita della dir che punta all’indice (I blocchi infatti non contengono informazioni sugli altri blocchi del file. In scrittura si aggiornano le directory in modo sincrono - non sempre risolve).
...
## Registrazione Modifiche (log) - Crash

- Se il sistema crolla, al momento del riavvio:
    - Si analizza il log e si eseguono tutte le transazioni registrate (queste modifiche non erano ancora state fatte sui metadati).
- Problema quando la transazione non è committed al momento del crash:
    - Tutti i cambiamenti fatti al FS annullati per mantenere consistenza (unico ripristino necessario).
- Il log è tipicamente salvato in un’area dedicata del disco.
    - Accedere in modo sequenziale e sincrono al log per salvare le transazioni (È più veloce che accedere in modo diretto e sincrono ai blocchi contenenti i meta-dati).
- Questa tecnica diminuisce il tempo in cui un processo resta bloccato in attesa dell’aggiornamento dei metadati.

## Registrazione Modifiche (log) - Flusso
