...
- Una **testina read-write** si move sui suoi piatti.
- Il braccio muove le testine.
...
- I primi settori erano da 512 bytes, ora fino a 4 KB.

---

## Hard Disk (Specifiche Tecniche)
...
- **Performance**:
    - **Transfer Rate**: da disco a computer.
    - Teorico: 6 Gb/sec.
    - **Effective Transfer Rate** (reale): 1 Gb/sec.
- **Tempo di posizionamento**:
    - **Seek time** e **rotational latency**.
    - Seek time da 3ms a 12ms (9ms comune per desktop drives).
    - La latenza dipende dalla **spindle speed**.
    - Formula: 1 / (RPM / 60) = 60 / RPM.
...
□ Esempio: trasferire un blocco di 4KB su un disco da 7200 RPM con un average seek time di 5ms, 1Gb/sec di transfer rate con un .1ms overhead del controllore:
...
I dispositivi **NVM** sono sempre più rilevanti e diffusi. Sono **elettrici e non meccanici**. 

Tipicamente composti basati su **memoria flash (chip NAND flash)**, vengono spesso inseriti in contenitori simili a unità disco, e per questa ragione sono chiamati **dischi a stato solido, o SSD**.
...
- In generale ha meno capacità di HDD ma è più veloce.
...
- **Non parti mobili, non parti meccaniche**:
    - **non seek time** e **non rotational latency**.
...
I semiconduttori NAND **non possono essere sovrascritti direttamente**, devono prima essere cancellati. Sono organizzati in pagine e presentano pagine che contengono dati non validi.

La **cancellazione avviene in blocco** e prende tempo (> del tempo di scrittura > tempo lettura). Le operazioni possono avvenire in parallelo. Deterioramento dopo ogni cancellazione. 

Durata misurata in **Drive Writes per Day (DWPD)**: quanto volte può essere scritta per giorno prima del fallimento (es. 1 TB può avere 5 DWPD nel periodo garantito).
...
Per gestire i dati occorrono: 
- Algoritmi di **garbage collection** per liberare blocchi da pagine valide.
- Pagine (circa 20%) sempre disponibili per fare il write (**overprovisioning**).
- Meccanismi per distribuire le cancellazioni.
- Meccanismi di correzione del dato (se errori continui su una pagina, questa è marcata come bad).
...
- Usati soprattutto per **backup**, stoccaggio di dati non usati frequentemente, trasferimento tra sistemi.
...
## Mapping degli Indirizzi (Dettagli)
...
## Mapping degli Indirizzi (Gestione Fisica)
...
- Il passaggio da indirizzi logici a fisici dovrebbe essere facile ma …
    ...
    - Settori possono essere danneggiati.
...
## Struttura Disco (Velocità e Dimensioni)
...
## Scheduling del Disco (Criteri e Code)
...
## Scheduling del Disco (Esempio di Coda)
...
- Le richieste di servizio possono essere influenzate dai metodi di file-allocation:
    - File allocato in modo **contiguo** $\rightarrow$ meno movimenti di testina.
    - File **sparsi** $\rightarrow$ più movimenti di testina.
...
## Selezionare un Algoritmo di Disk-Scheduling (Linux)
...
## Selezionare un Algoritmo di Disk-Scheduling (NOOP)
...
- **Vantaggi**: Scheduler semplicissimo e molto efficiente in termini di esecuzione. Non ha nessuna logica di riduzione dei seek (a parte il merging) ideale per dispositivi non rotazionali $\rightarrow$ (**SSD**).
- **Svantaggi**: Non ha nessuna logica di riduzione dei seek (a parte il merging) disastroso sui dispositivi rotazionali $\rightarrow$ **Starvation** delle richieste. Incapacità di differenziare in base all’importanza del processo.
...
- I/O possono avvenire in modo random o sequenziale:
    ...
    - **input/output operations per second (IOPS)** diversi per HDD e NVM:
        - Nel caso di accesso random: centinaia vs centinaia di miglia.
...
□ Il dispositivo deve essere strutturato:
    ...
    - Formattazione di basso livello o fisica, disco in settori che il disk controller può leggere e scrivere (produttore fornisce e fa testing).
...
## Error Detection and Correction (Esempio)
...
## Gestione del Disco (Strutturazione Logica)
...
## Gestione del Disco (Volumi e File System)
...
- Per incrementare l’efficienza i file system raggrupano i blocchi in **clusters**.
...
## Gestione del Disco (Esempi Windows)
...
## Gestione del Disco (Esempi Windows 10)
...
## Booting da disco GPT (Processo UEFI)
...
- Partizione può essere un volume che contiene un file system (“cotta”) o **raw (cruda)** – una sequenza di blocchi senza un file system.
...
## Gestione dello Swap-Space (Esempio UNIX)

- Nei primi sistemi il Kernel faceva swapping dell’intero processo in locazioni contigue del disco, poi si è passati alla gestione con **paging**.
...
- In Solaris 2 alloca swap space solo quando una **dirty page** viene mandata fuori dalla memoria fisica (non quando è creata la pagina della virtual memory).
...
- Nel mirroring duplicate le scritture, però le richieste di lettura sono solo ad una delle unità e questo non migliora il trasferimento.
...
## Redundant Array of Inexpensive Disks (Livelli)
...
*(a) RAID 0: non-redundant striping. (b) RAID 1: mirrored disks. (c) RAID 2: memory-style error-correcting codes. (d) RAID 3: bit-interleaved parity. (e) RAID 4: block-interleaved parity. (f) RAID 5: block-interleaved distributed parity. (g) RAID 6: P + Q redundancy.*
...
- **RAID 5** distribuisce i parity su tutti i dischi, evita di usare troppo il disco parity.
    ...
    - ECC usato con stripping (Il primo blocco in drive 1, secondo in drive 2, N in drive N, error in N + 1).
...
- **RAID 10 (1 + 0)**:
    - Prima dati duplicati poi striped sui dischi (almeno 4).
...
*(a) Traditional volumes and file systems. (b) ZFS and pooled storage.*