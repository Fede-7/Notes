...
Delle funzioni che vedremo sarà ricorrente il parametro "y" che altro non è che la codifica (vista nelle sezioni precedenti) del programma che stiamo considerando. Se ad esempio stiamo considerando il programma che calcola la funzione somma, y sarà la codifica delle istruzioni del programma S che calcolerà la somma tra x1 e x2.
...
\section{Come utilizzare le funzioni HALT, STP e Universale}

Abbiamo introdotto tre importanti concetti: la funzione universale, il predicato HALT e il predicato STP.

La prima differenza è che HALT non è mai calcolabile (quindi non possiamo usarlo nei programmi S), STP invece è addirittura ricorsiva primitiva quindi possiamo usarlo liberamente, mentre la funzione universale è parzialmente calcolabile quindi possiamo usarla nei programmi S ma non possiamo essere sicuri che termini sempre.

La funzione HALT ci permette di capire se il programma y con i parametri x1, x2, ..., xn termina o meno. Il predicato STP invece ci permette di capire se il programma y con i parametri x1, x2, ..., xn termina in al più t passi.
Mentre il programma universale ci permette di simulare il programma y con i parametri x1, x2, ..., xn. 

Il programma S seguente, è la funzione somma che calcola la somma di X1 e X2 e la mette in Y. La identifichiamo come f(x1, x2).
...
Come visto nelle sezioni precedenti, per poter usare funzioni come STP e la funzione universale dobbiamo codificare l'S-Programma in un numero, ricaviamo la nostra y.

Il programma $\mathcal{P}$ viene codificato tramite la funzione di Gödel. Definiamo la quaterna di codifica per ogni istruzione $I_k$ come $\#(I_k) = \langle a, b, c, d \rangle$. Utilizzando la mappatura dei registri $R_1=Y, R_2=Z, R_3=X_1, R_4=X_2$, otteniamo:

\begin{itemize}
    \item $n_1 = \#(Y \leftarrow X_1) = \langle 0, 0, 1, 3 \rangle$
    \item $n_2 = \#(Z \leftarrow X_2) = \langle 0, 0, 2, 4 \rangle$
    \item $n_3 = \#([A] \text{ IF } Z \neq 0 \text{ GOTO } B) = \langle 1, 4, 2, 2 \rangle$
    \item $n_4 = \#(\text{GOTO } E) = \langle 0, 2, 0, 0 \rangle$
    \item $n_5 = \#([B] Z \leftarrow Z - 1) = \langle 2, 2, 2, 0 \rangle$
    \item $n_6 = \#(Y \leftarrow Y + 1) = \langle 0, 1, 1, 0 \rangle$
    \item $n_7 = \#(\text{GOTO } A) = \langle 0, 3, 0, 1 \rangle$
\end{itemize}

Il parametro $z$ da utilizzare nei nostri programmi sarà (per semplicità non calcoliamo il numero di Gödel completo ma lasciamo la quaterna di codifica di ogni istruzione): 
\[ z = \langle n_1, n_2, n_3, n_4, n_5, n_6, n_7 \rangle \]

Ok ora possiamo scrivere un programma S che valuta se il programma di somma termina in al più t passi (nel programma i passi t sono Z0) e, in tal caso, ne calcola il risultato ad esempio.

\begin{lstlisting}[language=ini, label={Funzione Somma},  caption=Funzione somma]
  Z1 <- 0 # Inizializziamo il contatore dei passi  
  [A] Z2 <- STP(X1, X2, z, t) # Il parametro z è ciò che abbiamo trovato nei passi precedenti

  IF Z2 != 0 GOTO B
  Z1 <- Z1 + 1 # Se il programma non è terminato in Z1 passi allora incrementiamo Z1 e ricominciamo
  GOTO A

  [B] Y <- f(X1, X2) # Sappiamo che f termina in al più Z1 passi, quindi calcoliamo il risultato

\end{lstlisting}

La funzione universale si usa in modo simile, ma invece di verificare se il programma termina in al più t passi, lo esegue direttamente. Quindi il programma S diventa:

\begin{lstlisting}[language=ini, label={Funzione Somma},  caption=Funzione somma]
  
  Y <- Φ(X1, X2, z) # Il parametro z è ciò che abbiamo trovato nei passi precedenti

\end{lstlisting}

Per inciso, in questo specifico esempio era inutile usare la funzione STP che cercava di capire se in al più t passi il programma terminasse poiché sappiamo già che la funzione f(x1, x2) termina sempre. Ma ai fini didattici è utile per capire come usare queste funzioni.

\section{Esercizi dalle prove d'esame su STP, HALT e Funzione Universale}

\subsection{Esercizio 1}

Sia $f$ una funzione binaria totale tale che $f(x, y) > 0 \iff \exists z : \mathrm{STP}(x, y, z)$. Dimostrare che $f$ non può essere calcolabile.

\textbf{Soluzione:} Qui non c'è bisogno di creare un programma ma solo usare qualche definizione e proprietà viste nel capitolo.

Supponiamo per assurdo che \(f\) sia calcolabile. Allora $\mathrm{STP}{x,y,z}$ sarebbe calcolabile ma STP è scrivibile anche come $\mathrm{HALT}(x,y)$. Se ciò fosse vero allora avremmo risolto il problema della fermata, che sappiamo essere indecidibile. Quindi la nostra ipotesi iniziale è falsa e \(f\) non può essere calcolabile.

\subsection{Esercizio 2}

Sia $h$ la funzione binaria così definita:
$$
h(x_1, x_2) = \begin{cases}
(x_1, x_2) & \text{se } \exists z : \mathrm{STP}(x_1, x_2, z) \\
\uparrow & \text{altrimenti.}
\end{cases}
$$
Tale funzione è parzialmente calcolabile? Dimostrare o confutare.

\textbf{Soluzione:} Qui attenzione a non confondere i parmaetri di STP. Sappiamo che STP accetta: parametri, programma, passi. 
Quindi in questo caso $x_1$ è il parametro, $x_2$ il programma (numero che ci viene già fornito quindi non dobbiamo calcolarlo noi) e $z$ i passi.
Fatta questa doverosa precisazione, possiamo scrivere l'S-programma che calcola la funzione \(h\):

\begin{lstlisting}[language=ini, label={Funzione h},  caption=
Funzione h]
  Z1 <- 0 # Inizializziamo il contatore dei passi  
  
  # Qui stiamo cercando di capire se il programma X2 con parametri X1 termina in al più Z1 passi. 
  # Dovesse essere così, allora potremmo usare liberamente il programma universale per calcolare il risultato.
  [A] Z2 <- STP(X1, X2, Z1)
  ...
  [B] Y <- $\Phi^{(2)}(x_1,x_2)$ # Sappiamo già che il programma X2 con parametri X1 termina in al più Z1 passi (grazie ad STP), quindi calcoliamo il risultato

\end{lstlisting}

% ------

\section{Esercizi e approfondimenti}
