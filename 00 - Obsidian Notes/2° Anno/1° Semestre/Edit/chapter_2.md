...
> - La funzione nulla $n(x)=0$;
> - La funzione successore $s(x)=x+1$;
> - Le funzioni proiezione $u_i^{(n)}(x_1,\cdots,x_n)=x_i, \forall (n\geq 1 \land 1\leq i \leq n )$.
...
[^8]: La funzione $f(x,y)=|x-y|$ è definita e dimostrata come ricorsiva primitiva nell'esercizio*(ref: Funzione Sottrazione Valore Assoluto Eq)*. Esercizi consigliati:*(ref: Funzione Sottrazione Valore Assoluto Eq)*,*(ref: Predicato Minore Uguale Eq)*
