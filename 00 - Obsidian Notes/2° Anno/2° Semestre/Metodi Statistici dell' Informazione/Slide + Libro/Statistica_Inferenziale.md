...
La statistica inferenziale può essere contrapposta alla statistica descrittiva. La statistica descrittiva si occupa esclusivamente delle proprietà dei dati osservati e non si basa sull'assunzione che i dati provengano da una popolazione più ampia.
...
Così, la media campionaria converge con probabilità uno alla media statistica della popolazione.Questa proprietà è anche definita nelle statistiche inferenziali come forte coerenza.
...
Abbiamo quindi un insieme di $M$ ipotesi diverse e mutuamente esclusive $\{ H _ { i } \} _ { i = 1 } ^ { M }$, ciascuna delle quali definisce una diversa legge condizionale per il set di dati, ovvero:

$$
p _ {\boldsymbol {X} ^ {n}} \left(\boldsymbol {x} ^ {n} \mid H _ {i}\right), \quad i = 1, \dots , M
$$

Assumiamo che il vettore casuale $X ^ { n }$ sia estratto da una famiglia di distribuzioni con pmf $p _ { X ^ { n } | \Theta } ( { \pmb x } ^ { n } | \theta )$, dove il valore di $\theta$ è sconosciuto;

Assumiamo anche che le probabilità a priori - $\{ p ( H _ { i } ) \} _ { i = 1 } ^ { M }$ - di questi stati della natura siano assegnate;
...
Assumiamo di definire la seguente matrice di costo $M \times M$
...
dove $C _ { i , j }$ è il costo associato all'evento in cui prendiamo la decisione $D ( \pmb { x } ^ { n } ) = i$ e lo stato della natura è $H _ { j }$
...
Data una matrice di costo $C$, una regola di decisione ottimale è una mappa $D ( \pmb { x } ^ { n } )$ che minimizza il rischio Bayesiano;

Si noti che, se $C _ { i , i } = 0 ~ i = 1 , . . .$, $M$ e $C _ { i , j } = 1 \forall i \neq j$, allora
...
Assumiamo per il momento che $M = 2$, che $C _ { 1 , 1 } = C _ { 2 , 2 } = 0$ e $C _ { 1 , 2 } = C _ { 2 , 1 } = 1$ in modo che

$$
\mathcal {R} = \mathbb {P} \left\{D (\boldsymbol {X} ^ {n}) = 2, H _ {1} \right\} + \mathbb {P} \left\{D (\boldsymbol {X} ^ {n}) = 1, H _ {2} \right\} = \mathbb {P} (e)
$$

Progettare una regola di decisione implica determinare una partizione di $\mathcal { X } ^ { n }$ in due sottoinsiemi, $\Omega _ { 1 }$ e $\Omega _ { 2 }$, tali che
...
Esistono numerose situazioni in cui dobbiamo prendere una decisione tra due ipotesi, ma non abbiamo mezzi per assegnare la matrice di costo C né le probabilità a priori; 
...
È inoltre di poca importanza assegnare una probabilità a priori che "anomalie statistiche" siano presenti nel set di dati. 
...
Si noti che l'applicazione di qualsiasi funzione monotonicamente crescente a entrambi i lati del test precedente non ne altera l'ottimalità, per cui possiamo introdurre equivalentemente il log-likelihood ln $L \left( \pmb { x } ^ { n } \right) = \Lambda ( \pmb { x } ^ { n } )$ e confrontarlo con una soglia determinata in modo nuovo.
...
θ è tipicamente un parametro continuo, che può essere una realizzazione di una variabile casuale continua Θ con marginale noto $f _ { \Theta } ( \theta )$ (impostazione Bayesiana) o una quantità deterministica sconosciuta che assume valori in un insieme continuo;
...
Si noti che, se $\Theta$ è discreto, quanto sopra diventa un problema di classificazione. Si noti inoltre che nell'equazione sopra abbiamo usato il fatto che
...
Facendo la media su B otteniamo la legge incondizionata: 
...
Sfruttando la disuguaglianza di Tchebyshev, abbiamo che entrambi gli estimatori tendono a B in probabilità (consistenza, nota anche come consistenza debole). Se ${ \widehat { B } } ( X ^ { n } )$ è uno dei due estimatori, abbiamo quindi: 
...
Detto in altro modo, il prior coniugato della media di una distribuzione Gaussiana è di nuovo Gaussiano; 
...
Annullando il gradiente rispetto ad a e la derivata rispetto a $b$ otteniamo: 
...
Calcoli lunghi, sebbene semplici, portano alla forma LMS generale
